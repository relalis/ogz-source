...
//	RSetGammaRamp(Z_VIDEO_GAMMA_VALUE);
	...
	if( !g_pDefFont->Create("Default", "굴림", 9, 1.0f) )
		//	if( !g_pDefFont->Create("Default", RGetDevice(), "돋움", 9, 1.0f, true, false) )
		//	if( !g_pDefFont->Create("Default", RGetDevice(), "돋움", 14, 1.0f, true, false) )
	{
		g_pDefFont->Destroy();
